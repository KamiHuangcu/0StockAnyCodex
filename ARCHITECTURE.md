...
│   ├── init_db(conn)         ← 建立 8 張表 + 2 個 View + 索引
...
│       ├── bar_time.date() < today → INSERT OR IGNORE（歷史凍結，不覆蓋）
│       ├── bar_time.date() == today → INSERT OR REPLACE（當日刷新）
│       └── DELETE 過期資料（bar_time < today - KEEP_DAYS[interval_type]）
│
├── [指標計算模組] 合併兩個程式的最佳版本
...
3. DB 模組（`init_db` 建立 8 張表 + 2 個 View + 索引）
   - 8 張表：`symbols`, `k_bars`, `k_bar_features`, `chip_daily`, `ai_labels`,
     `stock_fundamentals`, `market_context`, `finmind_fetch_log`
